...
$n_mois = isset($_GET["mois"]) ? (int)$_GET["mois"] : null;
$ref = $_GET["ref"] ?? null;

// Traitement du formulaire (ajout ou modification)
if (isset($_POST["n_mois"]) && isset($_POST["reference_p"]) && isset($_POST["quantite"])) {
  $n_mois_post = (int)$_POST["n_mois"];
  $reference_p = $_POST["reference_p"];
  $quantite = (int)$_POST["quantite"];

  if ($action === "modifier" && $n_mois !== null && $ref !== null) {
    if ($n_mois_post === $n_mois && $reference_p === $ref) {
      DAO::edit_besoin($n_mois, $ref, $quantite);
    } else {
      if (DAO::get_besoin($n_mois_post, $reference_p)) {
      } else {
        DAO::delete_besoin($n_mois, $ref);
        DAO::add_besoin($n_mois_post, $reference_p, $quantite);
      }
    }
  } else {
    DAO::add_besoin($n_mois_post, $reference_p, $quantite);
...
// Suppression
if ($action === "supprimer" && $n_mois !== null && $ref !== null) {
  DAO::delete_besoin($n_mois, $ref);
  header("Location: besoin.php");
  exit;
}
...
<head>
  ...
  <title>Besoin - Labo Pharmaceutique</title>
...
<body>
  <header>
    ...
    <div class="nav-links">
      ...
      <a href="./besoin.php" class="active">Besoin</a>
  ...
  <main>
    ...
    if ($action !== "modifier") {
      ...
      echo '
        ...
        <div class="row-3">
          <input type="number" name="quantite" placeholder="Quantité à produire" min="1" required>
        </div>
        <button type="submit">
      ...
      $besoin = DAO::get_besoin($n_mois, $ref);
      $medicaments = DAO::get_medicaments();
      if ($besoin) {
        echo '<form action="?action=modifier&mois=' . urlencode($n_mois) . '&ref=' . urlencode($ref) . '" method="post">';
        echo '
          <h2>Modifier le besoin</h2>
          <div class="row-1 dropdown">
            <select name="n_mois" required>';
        for ($i = 1; $i <= 12; $i++) {
          $selected = ($i == $besoin["N_Mois"]) ? "selected" : "";
        ...
        if (is_array($medicaments)) {
          foreach ($medicaments as $med) {
            $selected = ($med["Reference_P"] == $besoin["Reference_P"]) ? "selected" : "";
            echo '<option value="' . htmlspecialchars($med["Reference_P"]) . '" ' . $selected . '>' . htmlspecialchars($med["Reference_P"] . ' - ' . $med["Designation"]) . '</option>';
          }
        }
        echo '
            </select>
          ...
          <div class="row-3">
            <input type="number" name="quantite" placeholder="Quantité à produire" min="1" value="' . htmlspecialchars($besoin["Quantite"]) . '" required>
          </div>
          <button type="submit">
